...
У тебе НЕМАЄ жодних інструментів середовища: ти не читаєш файли, не запускаєш команди,
не ходиш у мережу. Єдиний спосіб щось зробити — назвати інструмент у JSON нижче;
виконає його той цикл, що тебе викликав, і поверне результат наступним повідомленням.
Спроба скористатись власним інструментом обриває виклик помилкою.
...
class ClaudeCodeChatModel(BaseChatModel):
    ...
    def _system_prompt(self, messages: list[BaseMessage]) -> str:
        ...
        if not self.tools:
            ...
            # модель зводить у щось третє. Прибираємо свій.
            return "\n\n---\n\n".join(parts)
        forced = ""
